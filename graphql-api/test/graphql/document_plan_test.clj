...
(deftest ^:integration create-document-plan-test
  (queries/validate-resp (graph/nlg (queries/create-document-plan {:id "0" :uid "0" :name "test"
                                                                   :blocklyXml "<>" :documentPlan "{}"}))))

(deftest ^:integration update-document-plan-test
  (queries/validate-resp (graph/nlg (queries/update-document-plan {:id "0" :uid "0" :name "test-updated"
                                                                   :blocklyXml "<>" :documentPlan "{}"}))))
