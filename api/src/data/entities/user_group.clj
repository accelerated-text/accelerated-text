...
(defn link-dict-item [group-id dict-item-ref]
  (let [group-info (get-or-create-group group-id)
        linked-items (get group-info ::user-group/dictionary-items [])]
    (log/debugf "Linking dictionary-item: `%s` to user group ID: %d" dict-item-ref group-id)
    (db/update! user-group-db group-id (assoc group-info ::user-group/dictionary-items (conj linked-items [::dictionary-item/id dict-item-ref])))))

(defn link-reader-model [group-id reader-model-ref]
  (let [group-info (get-or-create-group group-id)
        linked-readers (get group-info ::user-group/reader-models [])]
    (log/debugf "Linking reader model: `%s` to user group ID: %d" reader-model-ref group-id)
    (db/update! user-group-db group-id (assoc group-info ::user-group/reader-models (conj linked-readers [::reader-model/code reader-model-ref])))))
